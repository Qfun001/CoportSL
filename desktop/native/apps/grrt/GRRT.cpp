...
#include "apps/grrt/Postprocess.h"
...
std::vector<fluid::FrameInfo> select_output_frames(
    std::span<const fluid::FrameInfo> available,
    std::string_view task) {

    if (available.empty()) {
        throw std::invalid_argument(
            std::string(task) + " has no available output frames.");
    }
    if (Config::FRAME_START < 0) {
        return {available.begin(), available.end()};
    }
    std::vector<fluid::FrameInfo> selected;
    for (const fluid::FrameInfo& frame : available) {
        if (frame.index >= Config::FRAME_START &&
            frame.index <= Config::FRAME_END) {
            selected.push_back(frame);
        }
    }
    if (selected.empty() || selected.front().index != Config::FRAME_START ||
        selected.back().index != Config::FRAME_END) {
        throw std::invalid_argument(
            std::string(task) + " output frame range " +
            std::to_string(Config::FRAME_START) + ".." +
            std::to_string(Config::FRAME_END) +
            " must lie inside the available range " +
            std::to_string(available.front().index) + ".." +
            std::to_string(available.back().index) + ".");
    }
    return selected;
}
...
slow_light::regions::RegionSelection selected_region(
    ...
    if (SlowLight::REGION_MODE == SlowLight::RegionMode::Suggest) {
        if (analysis == nullptr) {
    ...
    const auto item = std::find_if(
        ...
        [](const auto& candidate) {
            return candidate.name == SlowLight::MANUAL_SET;
        });
...
void run_fast(
    ...
    if (!resumed) {
        grrt::write_result_config(
            run, sequence, signatures, nullptr, nullptr, output_frames);
    }
    int completed = 0;
    for (const fluid::FrameInfo& frame : output_frames) {
        ...
        event::progress(
            ++completed, static_cast<int>(output_frames.size()), frame.index);
    }
}
...
void run_slow(
    ...
    if (!resumed) {
        grrt::write_result_config(
            run, sequence, signatures, &selection, &analysis, output_frames);
    }
    ...
    fluid::FrameCache cache = fluid::FrameCache::load(
        sequence, output_frames.front(), selection.left, selection.right);
...
void begin_resumed_run(
    ...
    std::cout << "Resuming " << grrt::task_name()
        << " result: " << run.path
        << " output_start=" << output_frames.front().index
        << " output_end=" << output_frames.back().index
        << " total=" << output_frames.size() << "\n";
    event::stage("resuming");
    event::progress(
        0, static_cast<int>(output_frames.size()), output_frames.front().index);
}
...
void run_region_error(
    ...
    for (const fluid::FrameInfo& frame : frames) {
        ...
        event::progress(++completed, static_cast<int>(frames.size()), frame.index);
    }
...
int run() {
    ...
    try {
        ...
        if (Config::TASK == Config::Task::Analysis) {
            ...
            event::result(analysis.run.path, analysis.reused);
            postprocess::run(analysis.run.path);
            return 0;
        }
        else if (Config::TASK == Config::Task::Fast) {
            const std::vector<fluid::FrameInfo> output_frames =
                select_output_frames(sequence.frames, "Fast");
            bool resumed = false;
            if (const auto existing = grrt::find_fast_result(
                sequence, output_frames, signatures)) {
            ...
            const Geometry geometry = build_geometry(sequence);
            run_fast(
                run, sequence, output_frames,
                geometry, signatures, resumed);
        }
        else if (Config::TASK == Config::Task::Slow) {
            ...
            const slow_light::regions::RegionSelection selection =
                selected_region(&*analysis);
            const auto slow_selection = slow_light::analysis::prepare_selection(
                sequence, geometry.rays, selection, {});
            std::cout << std::setprecision(17)
                << "Slow-light selection: name=" << slow_selection.regions.name
                << " window=" << slow_selection.window
                << " left=" << slow_selection.left
                << " right=" << slow_selection.right << "\n";
            ...
            const std::vector<fluid::FrameInfo> output_frames =
                select_output_frames(safe_frames, "Slow");
            bool resumed = false;
            if (const auto existing = grrt::find_slow_result(
                sequence, output_frames, signatures, slow_selection)) {
                if (existing->complete) {
            ...
            print_config(sequence, signatures, &run, &*analysis);
            run_slow(
                run,
                sequence,
                output_frames,
            ...
            print_config(sequence, signatures, &run);
            run_region_error(
                run, sequence, geometry, signatures, SlowLight::REGION_SETS);
        }

        run_io::write_run_status(run, "complete", status_nt0, status_nt1);
        event::result(run.path, false);
