...
void write_list(std::ostream& out, const Range& values) {
...
void write_config(const run_io::RunDirectory& run, const std::string& text) {
...
bool benchmark_summary_complete(
    ...
    while (std::getline(input, line)) {
        ...
        if (std::find(found.begin(), found.end(), key) != found.end()) return false;
        found.push_back(key);
    }
    for (int npix : BenchmarkConfig::NPIX_LIST) {
        for (int cores : core_counts) {
            if (BenchmarkConfig::RUN_FAST_LIGHT &&
                std::find(found.begin(), found.end(),
                    std::tuple{std::string("fast"), npix, cores}) == found.end()) {
                return false;
            }
            if (BenchmarkConfig::RUN_SLOW_LIGHT &&
                std::find(found.begin(), found.end(),
    ...
    const size_t lights = static_cast<size_t>(BenchmarkConfig::RUN_FAST_LIGHT) +
...
bool benchmark_result_matches(
    ...
    return actual == expected_config &&
        benchmark_summary_complete(config.parent_path() / "summary.csv", core_counts);
}
...
std::vector<int> supported_core_counts() {
    if (BenchmarkConfig::CORE_MODES.size() != 1) {
        throw std::runtime_error(
            "Benchmark requires exactly one core mode per run.");
    }
    if (BenchmarkConfig::CORE_MODES.front() ==
        BenchmarkConfig::CoreMode::AllLogical) {
        throw std::runtime_error(
            "Benchmark requires one OpenMP thread per physical core; "
            "AllLogical is not supported by this protocol.");
    }
...
void run_scan_for_resolution(
    ...
    write_scan_summaries(run, npix, grid_stats, frame_bytes, cases);
}
...
int run_benchmark() {
    ...
    try {
        ...
        if (!analysis.has_value()) {
            std::cout << "Benchmark analysis is missing; generating it outside "
                "the timed benchmark region.\n";
            const ray::RayGeometry analysis_rays = ray::build_ray_geometry(
                Config::NPIX, Config::FOV, Config::OBS,
                fluid::Backend::probe_grid);
            const fluid::GridLocations analysis_sampling =
                fluid::locate_ray_samples(analysis_rays);
            analysis = slow_light::analysis::run_analysis(
                sequence, analysis_rays, analysis_sampling, signatures);
        }
        ...
        run = run_io::allocate_run_directory(Config::OUTPUT / "benchmark");
        ...
        for (int npix : BenchmarkConfig::NPIX_LIST) {
            ...
            run_scan_for_resolution(
                run, sequence, analysis->suggested_regions, window,
                *first_item, grid_stats, frame_bytes, core_counts, npix);
        }
