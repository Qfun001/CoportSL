...
static void close_exact(FILE* file, const char* context) {
    if (fclose(file) != 0) {
        throw std::runtime_error(
            std::string("Cannot close BHAC ") + context + ".");
    }
}

static void scan_int_exact(FILE* file, int& value, const char* context) {
    char name[100] = {};
    char separator[100] = {};
    if (fscanf(file, "%99s %99s %d", name, separator, &value) != 3) {
        throw std::runtime_error(
            std::string("Cannot read BHAC grid ") + context + ".");
    }
}

static void scan_double_exact(FILE* file, double& value, const char* context) {
    char name[100] = {};
    char separator[100] = {};
    if (fscanf(file, "%99s %99s %lf", name, separator, &value) != 3) {
        throw std::runtime_error(
            std::string("Cannot read BHAC grid ") + context + ".");
    }
}
...
double read_grmhd_time(const char* fname) {
    ...
    if (file_id == NULL) {
        throw std::runtime_error(
            std::string("Cannot open BHAC frame header: ") + fname + ".");
    }
    std::unique_ptr<FILE, decltype(&fclose)> file_guard(file_id, &fclose);
    ...
    free(frame_nx);
    free(frame_neqpar);
    close_exact(file_guard.release(), "frame header");
    return header.t;
}
...
void init_grmhd_grid(char* fname, char* gridInName) {
    ...
    if (file_id == NULL) {
        throw std::runtime_error(
            std::string("Cannot open BHAC grid frame: ") + fname + ".");
    }
    else {
    ...
    if (inputgrid == NULL) {
        throw std::runtime_error(
            std::string("Cannot open BHAC grid input: ") + gridInName + ".");
    }
    std::unique_ptr<FILE, decltype(&fclose)> grid_guard(inputgrid, &fclose);

    scan_int_exact(inputgrid, nxlone[0], "nxlone1");
    scan_int_exact(inputgrid, nxlone[1], "nxlone2");
    if (ndimini == 3) {
        scan_int_exact(inputgrid, nxlone[2], "nxlone3");
    }
    scan_double_exact(inputgrid, xprobmin[0], "xprobmin1");
    scan_double_exact(inputgrid, xprobmin[1], "xprobmin2");
    if (ndimini == 3) {
        scan_double_exact(inputgrid, xprobmin[2], "xprobmin3");
    }
    scan_double_exact(inputgrid, xprobmax[0], "xprobmax1");
    scan_double_exact(inputgrid, xprobmax[1], "xprobmax2");
    if (ndimini == 3) {
        scan_double_exact(inputgrid, xprobmax[2], "xprobmax3");
    }
    scan_double_exact(inputgrid, hslope, "hslope");
    close_exact(grid_guard.release(), "grid input");
...
#endif
            if (i == (nleafs / 2) && c == 0) fprintf(stderr, ".");
        }
#pragma omp barrier
    }
    ...
    free(dx1);
    free(dxc);
    close_exact(file_guard.release(), "grid frame");
    fprintf(stderr, "Done\n");
}
...
void load_grmhd_frame(char* fname) {
    ...
    if (file_id == NULL) {
        throw std::runtime_error(
            std::string("Cannot open BHAC frame: ") + fname + ".");
    }
    else {
        fprintf(stderr, "\nSuccessfully opened %s. \n\nReading data", fname);
    }
...
#pragma omp barrier
    ...
    free(values);
    close_exact(file_guard.release(), "frame payload");
    fprintf(stderr, "Done\n");
}
...
int get_fluid_params_from_primitives(
    ...
    const double magnetic_norm = modvar->B_u[0] * modvar->B_d[0] +
        modvar->B_u[1] * modvar->B_d[1] +
        modvar->B_u[2] * modvar->B_d[2] +
        modvar->B_u[3] * modvar->B_d[3];

    // All GRMHD backends share the same density, magnetic-field, and electron-temperature scaling.
    fluid::set_radiative_scalars(
        *modvar, rho, uu, neqpar[0], magnetic_norm);
