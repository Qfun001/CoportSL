...
std::optional<double> kerr_schild_spin;

std::array<std::array<double, 4>, 4> kerr_schild_down(
    ...
    const double sin2 = std::pow(std::sin(theta), 2.0);
    const double rho2 = r * r + a2 * std::pow(std::cos(theta), 2.0);
    const double factor = 2.0 * r / rho2;
    metric[0][0] = -1.0 + factor;
    metric[0][1] = metric[1][0] = factor;
    metric[0][3] = metric[3][0] = -spin * sin2 * factor;
    metric[1][1] = 1.0 + factor;
    metric[1][3] = metric[3][1] = -spin * sin2 * (1.0 + factor);
    metric[2][2] = rho2;
    metric[3][3] = sin2 *
        (rho2 + a2 * sin2 * (1.0 + factor));
    return metric;
}

std::array<std::array<double, 4>, 4> kerr_schild_up(
    const std::array<double, 4>& x,
    double spin) {
    ...
    const double sin2 = std::pow(std::sin(theta), 2.0);
    const double rho2 = r * r + a2 * std::pow(std::cos(theta), 2.0);
    metric[0][0] = -1.0 - 2.0 * r / rho2;
    metric[0][1] = metric[1][0] = 2.0 * r / rho2;
    metric[1][1] = (r * (r - 2.0) + a2) / rho2;
    metric[1][3] = metric[3][1] = spin / rho2;
    metric[2][2] = 1.0 / rho2;
    metric[3][3] = 1.0 / (rho2 * sin2);
    return metric;
}
...
void SetKerrSchildMetric(double spin) {
    if (!std::isfinite(spin) || std::abs(spin) > 1.0) {
        throw std::invalid_argument("Kerr-Schild spin must be finite and |a| <= 1.");
    }
    kerr_schild_spin = spin;
}

void ResetMetric() {
    kerr_schild_spin.reset();
}
...
std::array<std::array<double, 4>, 4> MetricDown(const std::array<double, 4>& x) {
    ...
    if (kerr_schild_spin.has_value()) {
        return kerr_schild_down(x, *kerr_schild_spin);
    }
    ...
    if constexpr (metric == CAR) {
        ...
        constexpr  double a = ModelConstants::blackhole_spin;
        const double r = x[1];
        const double theta = x[2];
...
std::array<std::array<double, 4>, 4> MetricUp(const std::array<double, 4>& x) {
    ...
    if (kerr_schild_spin.has_value()) {
        return kerr_schild_up(x, *kerr_schild_spin);
    }
...
double get_radial_radius(const std::array<double, 4>& x) {
    ...
    if (kerr_schild_spin.has_value()) return x[1];
...
double get_polar_angle(const std::array<double, 4>& x) {
    ...
    if (kerr_schild_spin.has_value()) return x[2];
