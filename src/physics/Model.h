...
inline constexpr double blackhole_spin = Config::SPIN;
inline double& hs = Config::HS;
inline const double horizon =
    1.0 + std::sqrt(1.0 - blackhole_spin * blackhole_spin);

inline constexpr double R_source = Config::R_SOURCE;
...
inline constexpr int ElectronModel = Config::ELECTRON;

inline constexpr const char* electron_model_name() {
    if constexpr (ElectronModel == PowerLawModel) return "powerlaw";
    if constexpr (ElectronModel == BeamModel) return "beam";
    if constexpr (ElectronModel == LossConeModel) return "losscone";
    return "thermal";
}

inline constexpr double R_LOW = Config::R_LOW;
inline constexpr double R_HIGH = Config::R_HIGH;
inline constexpr double SIGMA_MAX = Config::SIGMA_MAX;
