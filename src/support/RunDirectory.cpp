...
std::optional<RunDirectory> find_latest_run(
    ...
    if (!std::filesystem::is_directory(category)) return std::nullopt;
        ...
        const bool matches = std::all_of(
            ...
            [&run](const auto& item) {
                return config_matches(
                    run.path / "config.txt", item.first, item.second);
            });
...
std::optional<RunDirectory> find_latest_complete_run(
    ...
    for (const RunDirectory& run : runs) {
        const bool config_complete = std::all_of(
            config_values.begin(), config_values.end(),
            [&run](const auto& item) {
                return config_matches(
                    run.path / "config.txt", item.first, item.second);
            });
        if (!config_complete) continue;
